...
public:
    ...
    virtual void MakeSharedJsEnv() override
    {
        ...
        if (NumberOfJsEnv > 1)
        {
			...
			// { @luochangan	
                JsEnvGroup = MakeShared<PUERTS_NAMESPACE::FJsEnvGroup>(NumberOfJsEnv,
                    std::make_shared<PUERTS_NAMESPACE::DefaultJSModuleLoader>(GetJsRootPath()), std::make_shared<puerts::FDefaultLogger>(),
            ...
            {
				// { @luochangan	
                JsEnvGroup = MakeShared<PUERTS_NAMESPACE::FJsEnvGroup>(NumberOfJsEnv, GetJsRootPath());
				// } @luochangan	
            }
        ...
        else
        {
            ...
            {
			// { @luochangan	
                JsEnv = MakeShared<PUERTS_NAMESPACE::FJsEnv>(std::make_shared<puerts::DefaultJSModuleLoader>(GetJsRootPath()),
            ...
            {
			// { @luochangan	
                JsEnv = MakeShared<PUERTS_NAMESPACE::FJsEnv>(GetJsRootPath());
			// } @luochangan		
            }
...
// { @luochangan	
    virtual const FString GetTsRootPath() override
    {
        return FPaths::ProjectContentDir() / TEXT("..") / GetDefault<UPuertsSetting>()->RootPath;
    }

    virtual const FString GetJsRootPath() override
    {
        return FPaths::ProjectContentDir() / TEXT("..") / GetDefault<UPuertsSetting>()->RootPath / TEXT("dist");
    }

    virtual const FString GetTypingGenerateRootPath() override
    {
        return FPaths::ProjectContentDir() / TEXT("..") / GetDefault<UPuertsSetting>()->TypeingGeneratePath;
    }

    virtual const int32 GetCommandLineDebugPort() override
    {
        return DebuggerPortFromCommandLine;
    }
// } @luochangan		
