...
void FPuertsEditorModule::OnPostEngineInit()
{
    if (Enabled)
    {
        ...
        // { @luochangan	
        JsEnv = MakeShared<PUERTS_NAMESPACE::FJsEnv>(std::make_shared<PUERTS_NAMESPACE::DefaultJSModuleLoader>(IPuertsModule::Get().GetJsRootPath()),
        ...
        JsEnv->Start("Editor/EditorEntry");
		// } @luochangan	
    }
}
